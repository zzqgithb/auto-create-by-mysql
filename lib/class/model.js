...
class jsModel {
    ...
    init() {
        ...
        this.column.forEach(item => {
            ...
            str += `this.${columnName} = ${this.normolizeData(columnName, dataType, columnComment, CHARACTER_MAXIMUM_LENGTH)}`
        })
        this.downLoadFile(str)
    }

    // 格式化数据返回
    normolizeData(columnName, dataType, columnComment, characterMaximumLength) {
        let entity = require('./entity')
        let model = new entity(columnName, columnComment, dataType, characterMaximumLength)
        let modelStr = '{\n'
