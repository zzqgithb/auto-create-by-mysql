...
<script type="text/ecmascript-6">
    import createSysUserSearchcount from '@/class/entity/SysUserSearchcount'
    import {SearchWrapper, ButtonWrapper, TablePanel, DialogForm} from '@/components/index'
    import * as sysUserSearchcountApi from '@/api/entityApi/sysUserSearchcount'
    ...
    export default {
        ...
        data() {
            return {
                ...
                formAttr: null, // 表单属性
                pageObj: null,
                entity: createSysUserSearchcount()
            }
        },
        created() {
            // 设置码表信息
        ...
        methods: {
            ...
            dialogSubmit(data) {
                if (this.dialogAttr.type === 'create') {
                    sysUserSearchcountApi.create(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
                } else if (this.dialogAttr.type === 'update') {
                    sysUserSearchcountApi.update(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
