...
<script type="text/ecmascript-6">
    import createSysSearchRelation from '@/class/entity/SysSearchRelation'
    import {SearchWrapper, ButtonWrapper, TablePanel, DialogForm} from '@/components/index'
    import * as sysSearchRelationApi from '@/api/entityApi/sysSearchRelation'
    ...
    export default {
        ...
        data() {
            return {
                ...
                formAttr: null, // 表单属性
                pageObj: null,
                entity: createSysSearchRelation()
            }
        },
        created() {
            // 设置码表信息
        ...
        methods: {
            ...
            dialogSubmit(data) {
                if (this.dialogAttr.type === 'create') {
                    sysSearchRelationApi.create(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
                } else if (this.dialogAttr.type === 'update') {
                    sysSearchRelationApi.update(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
