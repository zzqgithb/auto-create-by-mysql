...
<script type="text/ecmascript-6">
    import createQrtzTriggers from '@/class/entity/QrtzTriggers'
    import {SearchWrapper, ButtonWrapper, TablePanel, DialogForm} from '@/components/index'
    import * as qrtzTriggersApi from '@/api/entityApi/qrtzTriggers'
        ...
        methods: {
            ...
            dialogSubmit(data) {
                if (this.dialogAttr.type === 'create') {
                    qrtzTriggersApi.create(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
                } else if (this.dialogAttr.type === 'update') {
                    qrtzTriggersApi.update(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
