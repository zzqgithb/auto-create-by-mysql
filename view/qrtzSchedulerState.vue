...
<script type="text/ecmascript-6">
    import createQrtzSchedulerState from '@/class/entity/QrtzSchedulerState'
    import {SearchWrapper, ButtonWrapper, TablePanel, DialogForm} from '@/components/index'
    import * as qrtzSchedulerStateApi from '@/api/entityApi/qrtzSchedulerState'
    ...
    export default {
        ...
        data() {
            return {
                ...
                formAttr: null, // 表单属性
                pageObj: null,
                entity: createQrtzSchedulerState()
            }
        },
        created() {
            // 设置码表信息
        ...
        methods: {
            ...
            dialogSubmit(data) {
                if (this.dialogAttr.type === 'create') {
                    qrtzSchedulerStateApi.create(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
                } else if (this.dialogAttr.type === 'update') {
                    qrtzSchedulerStateApi.update(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
