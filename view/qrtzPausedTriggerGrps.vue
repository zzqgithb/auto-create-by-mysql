...
<script type="text/ecmascript-6">
    import createQrtzPausedTriggerGrps from '@/class/entity/QrtzPausedTriggerGrps'
    import {SearchWrapper, ButtonWrapper, TablePanel, DialogForm} from '@/components/index'
    import * as qrtzPausedTriggerGrpsApi from '@/api/entityApi/qrtzPausedTriggerGrps'
    ...
    export default {
        ...
        data() {
            return {
                ...
                formAttr: null, // 表单属性
                pageObj: null,
                entity: createQrtzPausedTriggerGrps()
            }
        },
        created() {
            // 设置码表信息
        ...
        methods: {
            ...
            dialogSubmit(data) {
                if (this.dialogAttr.type === 'create') {
                    qrtzPausedTriggerGrpsApi.create(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
                } else if (this.dialogAttr.type === 'update') {
                    qrtzPausedTriggerGrpsApi.update(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
