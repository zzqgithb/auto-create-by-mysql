...
<script type="text/ecmascript-6">
    import createSysProductMember from '@/class/entity/SysProductMember'
    import {SearchWrapper, ButtonWrapper, TablePanel, DialogForm} from '@/components/index'
    import * as sysProductMemberApi from '@/api/entityApi/sysProductMember'
        ...
        methods: {
            ...
            dialogSubmit(data) {
                if (this.dialogAttr.type === 'create') {
                    sysProductMemberApi.create(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
                } else if (this.dialogAttr.type === 'update') {
                    sysProductMemberApi.update(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
