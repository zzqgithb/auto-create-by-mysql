...
<script type="text/ecmascript-6">
    import createSysSearchClassification from '@/class/entity/SysSearchClassification'
    import {SearchWrapper, ButtonWrapper, TablePanel, DialogForm} from '@/components/index'
    import * as sysSearchClassificationApi from '@/api/entityApi/sysSearchClassification'
    ...
    export default {
        ...
        data() {
            return {
                ...
                formAttr: null, // 表单属性
                pageObj: null,
                entity: createSysSearchClassification()
            }
        },
        created() {
            // 设置码表信息
        ...
        methods: {
            ...
            dialogSubmit(data) {
                if (this.dialogAttr.type === 'create') {
                    sysSearchClassificationApi.create(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
                } else if (this.dialogAttr.type === 'update') {
                    sysSearchClassificationApi.update(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
            ...
            del() {
                ...
                sysSearchClassificationApi.del({
                    id: ids.toString()
                }).then(res => {
                    this.pageObj.getList()
                    this.$message.success(res.msg)
