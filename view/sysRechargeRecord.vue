...
<script type="text/ecmascript-6">
    import createSysRechargeRecord from '@/class/entity/SysRechargeRecord'
    import {SearchWrapper, ButtonWrapper, TablePanel, DialogForm} from '@/components/index'
    import * as sysRechargeRecordApi from '@/api/entityApi/sysRechargeRecord'
    ...
    export default {
        ...
        data() {
            return {
                ...
                formAttr: null, // 表单属性
                pageObj: null,
                entity: createSysRechargeRecord()
            }
        },
        created() {
            // 设置码表信息
        ...
        methods: {
            ...
            dialogSubmit(data) {
                if (this.dialogAttr.type === 'create') {
                    sysRechargeRecordApi.create(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
                } else if (this.dialogAttr.type === 'update') {
                    sysRechargeRecordApi.update(data).then(res => {
                        this.closeDialog()
                        this.pageObj.getList()
                        this.$message.success(res.msg)
                    })
